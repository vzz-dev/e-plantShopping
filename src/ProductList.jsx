      ...
      {!showCart ? (
        <div className="product-grid">
          {plantsArray.map(
            (
              category,
              index, // Recorre cada categoria del arreglo principal
            ) => (
              <div key={index}>
                {" "}
                {/* Clave unica para cada categoria */}
                <h1>
                ...
                <div className="product-list">
                  {" "}
                  {/* Contenedor de las plantas de esa categoria */}
                  {category.plants.map(
                    (
                      plant,
                      plantIndex, // Recorre cada planta dentro de la categoria
                    ) => (
                      <div className="product-card" key={plantIndex}>
                        {" "}
                        {/* Tarjeta individual de planta */}
                        {/* Imagen de la planta */}
                        <img
                        ...
                        <div className="product-cost">
                          {plant.cost}{" "}
                          {/* IMPORTANTE: no agregar $ porque ya viene el el array */}
                        </div>
                        {/* Boton para agregar al carrito */}
                        <button
                          className="product-button"
                          onClick={() => handleAddToCart(plant)} // Ejecuta la funcion pasando la planta
                          disabled={addedToCart[plant.name]} // Desactiva si ya fue agregado
                        >
